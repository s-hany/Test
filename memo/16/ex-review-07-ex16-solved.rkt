...
このファイルは「解答確認用」なので、解答コード記入箇所はすべて埋めてある。
        ...
        ((p1-exponentiation? exp)
         (p1-make-product
          (p1-make-product
           ...
           (p1-make-exponentiation
            (p1-base exp)
            (p1-make-sum (p1-exponent exp) -1)))
          (p1-deriv (p1-base exp) var)))
...
(define (p3-make-product m1 m2)
  (cond ((or (p3-=number? m1 0) (p3-=number? m2 0)) 0)
        ((p3-=number? m1 1) m2)
        ((p3-=number? m2 1) m1)
        ((and (number? m1) (number? m2)) (* m1 m2))
        (else (list m1 '* m2))))
...
(define (p5-encode-symbol sym tree)
  (cond ((p5-leaf? tree) '())
        ((memq sym (p5-symbols (p5-left-branch tree)))
         (cons 0
               (p5-encode-symbol sym (p5-left-branch tree))))
        ((memq sym (p5-symbols (p5-right-branch tree)))
         (cons 1
               (p5-encode-symbol sym (p5-right-branch tree))))
        (else
         (error "p5-encode-symbol: symbol not in tree" sym))))
...
(define (p6-successive-merge ordered-trees)
  (cond ((null? ordered-trees) '())
        ...
        (else
         (p6-successive-merge
          (p6-adjoin-set
           (p6-make-code-tree (car ordered-trees)
                              (cadr ordered-trees))
           (cddr ordered-trees))))))
...
(define (p7-make-from-mag-ang m a)
  (define (dispatch op)
    (cond ((eq? op 'real-part) (* m (cos a)))
          ...
          ((eq? op 'magnitude) m)
          ((eq? op 'angle) a)
          (else (error "Unknown op: P7-MAKE-FROM-MAG-ANG" op))))
  dispatch)
